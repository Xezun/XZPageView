...
#import "XZPageViewHorizontalManager.h"
#import "XZPageViewVerticalManager.h"
@import XZDefines;
@import ObjectiveC;
...
@implementation XZPageViewManager
...
- (instancetype)initWithPageView:(XZPageView *)pageView {
    ...
    if (self) {
        ...
        CGRect const bounds = pageView.bounds;
        ...
        _scrollView = [[XZPageScrollView alloc] initWithFrame:bounds pageView:pageView];
        ...
        [_scrollView setDelegate:pageView];
    }
    return self;
}
...
+ (XZPageViewManager *)managerForPageView:(XZPageView *)pageView direction:(XZPageViewDirection)direction {
    switch (direction) {
        case XZPageViewDirectionHorizontal:
            return [[XZPageViewHorizontalManager alloc] initWithPageView:pageView];
        case XZPageViewDirectionVertical:
            return [[XZPageViewVerticalManager alloc] initWithPageView:pageView];
    }
}
...
- (void)reloadData {
    ...
    _numberOfPages = [_pageView.dataSource numberOfPagesInPageView:_pageView];
    ...
    [self reloadCurrentPageView:bounds];
    [self layoutCurrentPageView:bounds];
    ...
    if (_currentPage != oldValue) {
        XZCallBlock(_didShowPageAtIndex, _pageView, _currentPage);
    }
    
    // 重启自动翻页计时器
    [self scheduleAutoPagingTimerIfNeeded];
}
...
- (void)delegateDidChange:(id<XZPageViewDelegate>)delegate {
    ...
    if ([delegate conformsToProtocol:@protocol(XZPageViewDelegate)]) {
        ...
        [self notifyDidTransitionPage:aClass];
    }
}
...
@implementation XZPageViewHorizontalManager
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView stopped:(BOOL)stopped {
    ...
    if (stopped) {
        if (PageWidth - contentOffsetX < 1.0 || -PageWidth - contentOffsetX > -1.0) {
            ...
            // 发送转场进度
            XZCallBlock(_didTransitionPage, _pageView, contentOffsetX, PageWidth, _currentPage, _reusingPage);
...
- (void)reloadCurrentPageView:(CGRect const)bounds {
    ...
    if (_currentPage >= _numberOfPages) {
        if (_currentPageView != nil) {
            _currentPageView = [_pageView.dataSource pageView:_pageView prepareForReusingView:_currentPageView];
        }
        return;
    }
    
    _currentPageView = [_pageView.dataSource pageView:_pageView viewForPageAtIndex:_currentPage reusingView:_currentPageView];
...
- (void)reloadReusingPageView:(CGRect const)bounds {
    ...
    if (_reusingPage == NSNotFound) {
        if (_reusingPageView != nil) {
            _reusingPageView = [_pageView.dataSource pageView:_pageView prepareForReusingView:_reusingPageView];
        }
        return;
    }
    
    _reusingPageView = [_pageView.dataSource pageView:_pageView viewForPageAtIndex:_reusingPage reusingView:_reusingPageView];
...
- (void)scheduleAutoPagingTimerIfNeeded {
    if (_numberOfPages <= 1 || _pageView.window == nil || _autoPagingInterval <= 0) {
        // 不满足计时器启动条件，销毁当前计时器。
        ...
        if (_autoPagingTimer.timeInterval != timeInterval) {
            [_autoPagingTimer invalidate];
            _autoPagingTimer = [NSTimer scheduledTimerWithTimeInterval:timeInterval target:self selector:@selector(autoPagingTimerAction:) userInfo:nil repeats:YES];
        }
        // 定时器首次触发的时间
...
- (void)setCurrentPage:(NSInteger)newPage animated:(BOOL)animated {
    ...
    if (animated) {
        ...
        [UIView performWithoutAnimation:^{
            ...
            [_scrollView setDelegate:_pageView];
        }];
        ...
        [UIView animateWithDuration:XZPageViewAnimationDuration animations:^{
            ...
            [self->_scrollView setDelegate:self->_pageView];
        }];
    }
}
